...
<!--                </el-col>-->
        ...
        <el-table :data="table_data">
            ...
            <el-table-column prop="is_dynamic_score" label="动态分数" min-width="25px">
                <template slot-scope="scope">
                    <el-switch @change="editBoolean('is_dynamic_score', scope.row.challenge_id, scope.row.is_open)" v-model="scope.row.is_dynamic_score"></el-switch>
                </template>
            </el-table-column>
            <el-table-column prop="is_open" label="可用" min-width="20px" >
                <template slot-scope="scope">
                    <el-switch @change="editBoolean('is_open', scope.row.challenge_id, scope.row.is_open)" v-model="scope.row.is_open"></el-switch>
                </template>
            </el-table-column>
...
<!--                            style="margin-right: 10px">-->
                    <!--                    </el-popover>-->
                    <el-tooltip content="flag管理"  placement="top">
                        <el-button type="" icon="el-icon-s-flag" plain circle @click.prevent="loadAttachment(scope.row)"></el-button>
                    </el-tooltip>
                    <el-button type="primary" icon="el-icon-edit" plain circle @click.prevent="editChallenge(scope)"></el-button>
                    <el-button type="danger" icon="el-icon-delete" plain circle @click.prevent="deleteChallenge(scope)"></el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog title="编辑题目" :close-on-click-modal="false" :visible.sync="showEditor">
            <el-form ref="editorForm">
                <el-form-item label="题目名" :required="true">
                    <el-input v-model="editor_title" autocomplete="off" style="margin-left: 1em; max-width: 220px;"></el-input>
                </el-form-item>
                <el-form-item label="分类" :required="true">
                    <el-select v-model="editor_category_id" placeholder="请选择" style="margin-left: 2em; max-width: 220px;">
                        <el-option
        ...
        <el-dialog title="附件管理" :close-on-click-modal="false" :visible.sync="show_attachment_editor">
            <el-form>
                ...
                <el-form-item>
                    ...
                    <el-row :gutter="20" class="attachment-row" v-for="(a, index) in attachment_list" :key="a.attachment_id">
                        <el-col :span="9">
                        ...
                        <el-col :span="6">
                            <el-button type="success" icon="el-icon-check" plain circle @click.prevent="editAttachment(a)"></el-button>
                            <el-button type="danger" icon="el-icon-delete" plain circle @click.prevent="deleteAttachment(a, index)"></el-button>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20" class="attachment-row">
                        ...
                        <el-col :span="6">
                            <el-button type="success" icon="el-icon-plus" plain circle @click.prevent="addAttachment"></el-button>
                        </el-col>
                    </el-row>
                </el-form-item>
            </el-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editAttachment(row) {
                api.post('/attachment/edit_attachment',{attachment_id: row.attachment_id, url: row.url, flag: row.flag}).then(() => {
                    this.$message({message: '保存成功', type: 'success'});
                }).catch((e) => {
            ...
            addAttachment() {
                ...
                api.post('/attachment/edit_attachment',{
                    ...
                    this.attachment_list.push({attachment_id: r, url: this.attachment_new_url, flag: this.attachment_new_flag, challenge_id:this.attachment_editor_challenge_id});
